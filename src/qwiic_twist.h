...
const uint8_t enableInterruptButtonBit = 1;
const uint8_t enableInterruptEncoderBit = 0;
...
{
public:
  TWIST(uint8_t deviceAddress = QWIIC_TWIST_ADDR);
  ...
  bool setCount(int16_t amount);                          //Set the number of indents to a given amount
  int16_t getDiff(bool clearValue = true);                //Returns the number of ticks since last check
  bool isMoved();                                         //Returns true if knob has been twisted
  bool isPressed();                                       //Return true if button is currently pressed.
  bool isClicked();                                       //Returns true if a click event has occurred. Event flag is then reset.
  uint16_t timeSinceLastMovement(bool clearValue = true); //Returns the number of milliseconds since the last encoder movement
  uint16_t timeSinceLastPress(bool clearValue = true);    //Returns the number of milliseconds since the last button event (press and release)
  void clearInterrupts();                                    //Clears the moved and clicked bits

  bool setColor(uint8_t red, uint8_t green, uint8_t blue); //Sets the color of the encoder LEDs, 0-255
  bool setRed(uint8_t);                                    //Set the red LED, 0-255
  bool setGreen(uint8_t);                                  //Set the green LED, 0-255
  bool setBlue(uint8_t);                                   //Set the blue LED, 0-255
  ...
  bool connectColor(int16_t red, int16_t green, int16_t blue); //Connect all colors in one command
  bool connectRed(int16_t);                                    //Connect individual colors
  bool connectGreen(int16_t);                                  //Connect individual colors
  bool connectBlue(int16_t);                                   //Connect individual colors
  int16_t getRedConnect();                                        //Get the connect value for each color
  int16_t getGreenConnect();
  int16_t getBlueConnect();

  bool setLimit(uint16_t); //Set the limit of what the encoder will go to, then wrap to 0. Set to 0 to disable.
  uint16_t getLimit();

  uint16_t getIntTimeout();                //Get number of milliseconds that must elapse between end of knob turning and interrupt firing
  bool setIntTimeout(uint16_t timeout); //Set number of milliseconds that elapse between end of knob turning and interrupt firing

  uint16_t getVersion(); //Returns a two byte Major/Minor version number

  bool changeAddress(uint8_t newAddress); //Change the I2C address to newAddress
